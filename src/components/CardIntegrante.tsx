...
export const CardIntegrante = ({ nome, rm, foto, github, linkedin }: IntegranteProps) => {
  const safeFoto = foto || 'https://via.placeholder.com/160';
    ...
    <div className="bg-white/5 backdrop-blur-lg border border-white/10 rounded-3xl p-8 transition-all duration-500 hover:-translate-y-2 hover:border-purple-500/50 group">
      <div className="relative mb-6 flex justify-center">
        ...
        <img 
          src={safeFoto} 
          alt={`Foto de ${nome}`} 
          className="relative h-40 w-40 rounded-full border-2 border-white/10 object-cover shadow-2xl" 
        />
      </div>
      ...
      <div className="flex gap-3">
        <a 
          ...
          className="flex-1 rounded-xl bg-white/10 py-2.5 text-center text-xs font-bold text-white hover:bg-white/20 transition-colors"
        >
          GitHub
        </a>
        <a 
          ...
          target="_blank" 
          rel="noopener noreferrer" 
          className="flex-1 rounded-xl bg-purple-600 py-2.5 text-center text-xs font-bold text-white hover:bg-purple-500 transition-all shadow-lg shadow-purple-900/20"
        >
          LinkedIn
        </a>
      </div>
